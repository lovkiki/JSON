...
public class JSON_Object {
	...
	public static JSON_Object Build(String json) {
		...
		for(int i = 0; i < cs.length; i++){
			if(cs[i] == '{') {
				...
				int t = findEnd(json.substring(cursor, json.length()), '[', ']');
				String[] parts = json.substring(cursor, cursor + t + 1).split(":");
				JSON_Object tObj;
				if(parts.length == 1){
					tObj = Build(parts[0]);
				}
	...
	public static void main(String[] args) {
		String json = "{\"a\": {\"aa\":\"hahaha\", \"bb\":{\"aabbc\": [2, \"ha!\", 3]}}, \"b\": 1, \"c\":[1,2,\"abc\", [[1,2,3],6,7,8]]}";
		JSON_Object object = JSON_Object.Build(json);
		System.out.println(object.select("a.bb.aabbc[1]"));
